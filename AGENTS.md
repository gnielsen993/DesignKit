...
3. Add the change under the appropriate section (Summary, API
...
6. Tag the release (`git tag vX.Y.Z`) only after the file is
   finalized and CI is green; the tag and the file ship together.

Semver: patch = bug fix / internal-only; minor = additive; major =
public API removed/renamed or default-behavior break.
